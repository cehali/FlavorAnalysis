...
def map_ingr(rec):
    ...
    for ingr in rec:
        ...
        ingredient = ingr.replace('[', '').replace(']', '').replace(" u'", '').replace(" '", '')\
            ...
            .replace('flounder', 'fish').replace('trout', 'fish').recipe('guanciale', 'pork').replace('tilapia', 'fish')\
...
for d in data:
    recipes_ingr_mapped_name.append(map_ingr(d.get('ingredientLines')))

recipes_ingr_mapped_name = np.array(recipes_ingr_mapped_name)
np.save('recipes_ingr_mapped_name', recipes_ingr_mapped_name)
